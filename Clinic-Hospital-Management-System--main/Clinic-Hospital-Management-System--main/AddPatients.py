...
def add_patient():
    ...
    def refresh_table():
        """Refresh the tree view with the current patients data."""
    ...
    root = Tk()
    root.title("LAKSHMI CLINIC MANAGEMENT SYSTEM")
    ...
    def add():
        ...
        MOBILE = str(MOBILEEntry.get()).strip()
        NAME = str(NAMEEntry.get()).strip()
        DOB = str(DOBEntry.get()).strip()
        HISTORY = str(HISTORYEntry.get()).strip()
        MEDICINES = str(MEDICINESEntry.get()).strip()

        if not all([MOBILE, NAME, DOB, HISTORY, MEDICINES]):
            messagebox.showinfo("Error", "Please fill up all fields")
            return

        try:
            with connection() as conn:
                cursor = conn.cursor()
                cursor.execute("INSERT INTO patients (MOBILE, NAME, DOB, HISTORY, MEDICINES) VALUES (?, ?, ?, ?, ?)", 
    ...
    def reset():
        """Reset all patient records in the database."""
        ...
        try:
            ...
            messagebox.showinfo("Error", f"Sorry, an error occurred: {str(e)}")
            return
    ...
    def select():
        """Select a patient record and fill in the entry fields."""
        try:
            selected_item = my_tree.selection()[0]
    ...
    def search():
        """Search for a patient record based on input fields."""
        MOBILE = str(MOBILEEntry.get()).strip()
        NAME = str(NAMEEntry.get()).strip()
        DOB = str(DOBEntry.get()).strip()
        HISTORY = str(HISTORYEntry.get()).strip()
    ...
    def update():
        ...
        try:
            with connection() as conn:
                cursor = conn.cursor()
                cursor.execute("UPDATE patients SET MOBILE=?, NAME=?, DOB=?, HISTORY=?, MEDICINES=? WHERE MOBILE=?", 
                               (MOBILE, NAME, DOB, HISTORY, MEDICINES, selectedMOBILE))
                conn.commit()
        except Exception as e:
            messagebox.showinfo("Error", f"An error occurred: {str(e)}")
            return
    ...
    # Buttons
    addBtn = Button(root, text="Add", padx=65, pady=25, width=10,
                    bd=5, font=('Arial', 15), bg="#84F894", command=add)
    updateBtn = Button(root, text="Update", padx=65, pady=25, width=10,
                       bd=5, font=('Arial', 15), bg="#84E8F8", command=update)
    deleteBtn = Button(root, text="Delete", padx=65, pady=25, width=10,
                       bd=5, font=('Arial', 15), bg="#FF9999", command=delete)
    searchBtn = Button(root, text="Search", padx=65, pady=25, width=10,
                       bd=5, font=('Arial', 15), bg="#F4FE82", command=search)
    resetBtn = Button(root, text="Reset", padx=65, pady=25, width=10,
                      bd=5, font=('Arial', 15), bg="#F398FF", command=reset)
    selectBtn = Button(root, text="Select", padx=65, pady=25, width=10,
                       bd=5, font=('Arial', 15), bg="#EEEEEE", command=select)

    backBtn = Button(root, text="Back", padx=65, pady=25, width=10,
    ...
    # Call create_table to ensure the table is ready
    create_table()
    refresh_table()
